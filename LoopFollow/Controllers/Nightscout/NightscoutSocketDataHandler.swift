...
extension MainViewController {
    func setupNightscoutSocket() {
        ...
        NotificationCenter.default.removeObserver(self, name: .nightscoutSocketStateChanged, object: nil)
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(handleNightscoutSocketStateChanged),
            name: .nightscoutSocketStateChanged,
            object: nil
        )

        updateWebSocketInfoRow()
        NightscoutSocketManager.shared.connectIfNeeded()
    }
    
    @objc func handleNightscoutSocketStateChanged() {
        updateWebSocketInfoRow()
    }

    private func updateWebSocketInfoRow() {
        let manager = NightscoutSocketManager.shared
        infoManager.updateInfoData(type: .websocket, value: manager.statusDisplayString)
        infoManager.setPriority(manager.statusIsPriority, for: .websocket)
    }
