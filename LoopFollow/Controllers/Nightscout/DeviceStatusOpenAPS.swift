...
extension MainViewController {
    func DeviceStatusOpenAPS(formatter: ISO8601DateFormatter, lastDeviceStatus: [String: AnyObject]?, lastLoopRecord: [String: AnyObject]) {
        ...
        if lastLoopRecord["failureReason"] != nil {
            ...
            if ((TimeInterval(Date().timeIntervalSince1970) - lastLoopTime) / 60) > 12 {
                LoopStatusLabel.text = " ᮰"
                LoopStatusLabel.textColor = UIColor(named: "LoopRed")
                latestLoopStatusString = "᮰"
                LogManager.shared.log(category: .deviceStatus, message: "Loop status 🔴 (\(lastLoopTime))", isDebug: true)

            }
            else if ((TimeInterval(Date().timeIntervalSince1970) - lastLoopTime) / 60) > 7 {
                LoopStatusLabel.text = " ᮰"
        ...
        //latestLoopTime = UserDefaultsRepository.alertLastLoopTime.value
    }
}
