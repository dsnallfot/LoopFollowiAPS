...
class MainViewController: UIViewController, UITableViewDataSource, ChartViewDelegate, UNUserNotificationCenterDelegate, UIScrollViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        UserDefaultsRepository.infoNames.value.append("CGM expiration")
        UserDefaultsRepository.infoNames.value.append("Pod expiration")
  ...
  */  
    ...
    func setBGTextColor() {
        if bgData.count > 0 {
            ...
            //Daniel: When toggle is off -> show  ordinary red-green-yellow BGText color. When toggle on, show Auggie's dynamic colors
            if !UserDefaultsRepository.colorBGText.value {
                ...
                color = UIColor(hue: hue, saturation: 0.8, brightness: 0.9, alpha: 1.0)
            }
